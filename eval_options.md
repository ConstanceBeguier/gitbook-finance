...
Dans un premier temps, nous supposons que l'action ne verse pas de dividende.
...
Si $$V(P_A, t) < V(P_B, t)$$, alors un arbitragiste ferait les transactions suivantes:
 ...
 - à $$T$$
   ...
   - vente de $$P_A$$
   - récupération des intérêts de son placement $$(V(P_B, t) - V(P_A, t))e^{r(T-t)}$$
...
Valeur du portefeuille B à $$T$$ noté $$V(P_B, T)$$:
 ...
 - si $$K > S_T$$, alors le put est exercé. Notre action est utilisée pour l'exercice du put. Donc $$V(P_B, T) = K$$.
...
Valeur du portefeuille A à $$t=0$$ : $$V(P_A, 0) = c + Ke^{-rT}$$

Valeur du portefeuille B à $$t=0$$ : $$V(P_B, T) = p + S_0$$
...
A $$\Delta t$$, le payoff de ce portefeuille est $$F_{\Delta t} - F_0$$. En actualisant cette valeur à $$t=0$$, nous obtenons une nouvelle valeur du portefeuille à $$t=0$$: $$e^{-r \Delta t} E[F_{\Delta t} - F_0]$$. Nous avons donc dans un univers risque-neutre, $$0 = e^{-r \Delta t} E[F_{\Delta t} - F_0]$$ soit $$E[F_{\Delta t}] = F_0$$. 

En faisant un raisonnement similaire sur les autres dates de réglement,  nous obtenons $$E[F_T] = 0$$ pour tout $$T$$. Le taux de croissance du prix future dans un univers-risque neutre est donc nul. $$F$$ suit donc le processus suivant $$dF = \sigma F dz$$. Le prix future se comporte donc comme le prix d'une action versant un taux de dividende $$q$$ égal au taux sans risque $$r$$. 
...
En l'absence d'opportunités d'arbitrage, une devise est analogue à une action payant un taux de dividende continu égal au taux sans risque dans la devise étrangère.Si nous notons: 
...
Le modèle de Black-Scholes suppose que la distribution de probabilité de l'actif sous-jacent suit une loi log-normale à toute date. Cette hypothèse est erronées. Les traders supposent que les queues de cette distribution sont asymétriques (plus épaisses à gauche qu'à droite). Pour tenir compte des écarts de distribution par rapport à la loi log-normale, les traders utilisent différentes courbes de volatilité:
 - le **smile de volatilité**: courbre représentant la volatilité implicite d'une option en fonction de son prix d'exercice
...
Pour rappel, la volatilité implicite d'un put est égale à la volatilité implicite du call ayant les mêmes caractéristiques (prix d'exercice et durée de vie).
...
Pour les options de change, le smile de volatilité est en forme de U. Cela signifie que les options très en dehors de la monnaie ou très dans la monnaie ont des volatilités implicites plus élevées que les options à la monnaie.
